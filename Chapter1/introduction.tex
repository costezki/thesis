...
Figure \ref{fig:mcg-graph-example-simple-structure} depicts the constituency division of the clause which is identical to the bracket notation in Example \ref{ex:2}. The nodes represent grammatical constituents and the edges stand for the part-whole composition. This constitutes the simplest structure that could be expected from a parser without any specification of constituent class, function or any other grammatical functions. 
We know as well from works on parsing with formal generative grammars that such composition can always be expressed as a tree (or parse tree). 
...
\begin{figure}[!ht]
	...
	\node (cla) [pattern-node, split-node, text width = 42em,]  
		{class:clause 
		\nodepart{two} configuration: posessive
		\nodepart{three}  tense:past simple, voice:active, polarity:positive};

		\node (sub) [pattern-node, split-node, below=3em of cla, xshift=-17em,] 
			{element:subject,\\class:pronoun
			\nodepart{two} participant-role:\\ agent
			\nodepart{three} person:3^{rd} pos:PRP\\ 
			number:singular\\gender:masculine\\word:He};

		\node (mv)[pattern-node, split-node, below=3em of cla, xshift=-6em] 
			{element:main verb \\class:verb
			\nodepart{two} process:\\ possessive
			\nodepart{three} pos:VBD\\word:gave};

		\node (com)[pattern-node,split-node, below=3em of cla, xshift=6em] 
			{element:complement,\\class:nominal group
			\nodepart{two} participant-role:\\affected-possessed
			\nodepart{three} animacy:inanimate\\number:singular\\gender:neutral };

		\node (det)[pattern-node,split-node, below=3em of com, xshift=-6em]
			{element:deictic,\\class:determiner
				\nodepart{two}
				\nodepart{three} pos:DT word:the};

		\node (nou)[pattern-node,split-node, below=3em of com, xshift=6em]
			{element:thing,\\class:noun
				\nodepart{two} 
				\nodepart{three} pos:NN word:cake};

		\node[pattern-node,split-node, below=3em of cla, xshift=17em](adjunct)
			{element:adjunct\\class:adverb
			\nodepart{two} circumstance:\\location
			\nodepart{three} pos:RB\\word:away};
